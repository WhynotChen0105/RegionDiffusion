...
from ldm.modules.diffusionmodules.text_grounding_tokinzer_input import GroundingNetInput
...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb, context, objs, region_masks):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb)
            elif isinstance(layer, SpatialTransformer):
                x = layer(x, context, objs, region_masks)
            else:
                x = layer(x)
        return x
...
class ResBlock(TimestepBlock):
    ...
    def _forward(self, x, emb):
        ...
        while len(emb_out.shape) < len(h.shape):
            emb_out = emb_out[..., None]  # 如果时间嵌入和z的输入的维度不同（一个是1d一个是2d），则把时间嵌入增加一维
...
class UNetModel(nn.Module):
    def __init__(
        ...
        self.only_middle_control = only_middle_control
        self.sd_v1_5 = sd_v1_5
        self.efficient_attention=efficient_attention
        assert fuser_type in ["gatedSA", "gatedRSA", "gatedCA"]
            ...
            for _ in range(num_res_blocks):
                ...
                if ds in attention_resolutions:
                    ...
                    layers.append(SpatialTransformer(ch, key_dim=context_dim, value_dim=context_dim, n_heads=num_heads,
                                                     ...
                                                     use_checkpoint=use_checkpoint, use_fuser=use_fuser, efficient_attention=efficient_attention))
        ...
        self.middle_block = TimestepEmbedSequential(
            ...
            SpatialTransformer(ch, key_dim=context_dim, value_dim=context_dim, n_heads=num_heads, d_head=dim_head,
                               ...
                               use_fuser=use_fuser,efficient_attention=efficient_attention),
            ResBlock(ch,
                     time_embed_dim,
                     dropout,
                     dims=dims,
    ...
    def forward(self, input):
        ...
        #     control = self.controlnet(input,objs, boxes, masks)

        # don't use fuser
        if not self.use_fuser:
            objs = None
            region_masks = None
        else:
            objs, region_masks = self.position_net(**grounding_input)
        # print(objs)
        # Start forwarding
        hs = []
        for module in self.input_blocks:
            h = module(h, emb, context, objs, region_masks)
            hs.append(h)

        h = self.middle_block(h, emb, context, objs, region_masks)

        # if control is not None:
        #     h += control.pop()  # 默认弹出列表的最后一个元素

        for module in self.output_blocks:
            # if self.only_middle_control or control is None:
